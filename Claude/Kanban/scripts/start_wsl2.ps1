...
$holder = 'pkill -f "pm-app-keepalive" 2>/dev/null; ' +
          '( exec -a pm-app-keepalive bash -c ' +
          '"while docker inspect -f {{.State.Running}} pm-app 2>/dev/null | grep -q true; do sleep 20; done" )'
