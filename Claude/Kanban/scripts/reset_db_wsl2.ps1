...
# (the persisted SQLite DB), then start again. On the next login the initial
...
wsl.exe --cd "$root" -- bash -lc "{ bash scripts/stop.sh; pkill -f pm-app-keepalive 2>/dev/null; true; } 2>&1"
